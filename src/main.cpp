...
int main(int argc, char* argv[]){
  ...
  try{
    ...
    std::pair<int, int> count = CountMalesOrFemalesByYear(file_artist, file_artist_type, file_gender, year);
    std::cout << "Number of male artists - " << count.first << ", number of female artists - " << count.second << std::endl;
  }
